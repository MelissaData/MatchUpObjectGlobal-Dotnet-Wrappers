# Name:    BuildMatchupNETdll
# Purpose: Compile the mdMatchupNET.dll wrapper
...
Write-Host "`n======================= WELCOME TO MELISSA MATCHUP OBJECT NET WRAPPER =======================`n"
...
cmd.exe /C """$CmdPath"" x86_x64 && csc -out:mdMatchupNET.dll -platform:AnyCPU -target:library mdMatchup_cSharpCode.cs" > $null

if (Test-Path ("mdMatchupNET.dll")) {
  $filePath = Join-Path -Path (Split-Path -Parent $PSCommandPath) -ChildPath "mdMatchupNET.dll"

  Write-Host "`nmdMatchupNET.dll has been successfully generated at:`n$filePath`n"
}
else {
  Write-Host "`nError: mdMatchupNET.dll was not created successfully`n"
